...
class SoftwareAgent:
    ...
    def process_task(self, task_data):
        ...
        if severity == "Critical":
            print(f"Critical task detected! Prioritizing task for incident {task_data['incident_id']}")
            # Handle critical tasks differently, if needed
        elif severity == "High":
            print(f"High priority task for incident {task_data['incident_id']}")
        elif severity == "Medium":
            print(f"Medium priority task for incident {task_data['incident_id']}")
        else:
        ...
        if "install" in task_data['task_description'].lower():
            print(f"Installing software for incident {task_data['incident_id']}")
        elif "troubleshoot" in task_data['task_description'].lower():
            print(f"Troubleshooting software for incident {task_data['incident_id']}")
        else:
            print(f"Performing general software maintenance for incident {task_data['incident_id']}")
